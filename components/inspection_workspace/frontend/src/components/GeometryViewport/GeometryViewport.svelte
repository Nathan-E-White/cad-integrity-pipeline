<script lang="ts">
 ...
 let {mesh,frame,ui,muted,traceId,onpick,onready}:{mesh:Mesh;frame:DisplayFrame;ui:WorkspaceState;muted:boolean;traceId:number|null;onpick:(target:Target|null)=>void;onready:(viewport:GeometryViewport|null)=>void}=$props();
 let host:HTMLDivElement;let viewport:GeometryViewport|undefined;let error=$state<string|null>(null);
 onMount(()=>{try{viewport=new GeometryViewport(host,mesh,frame,onpick,message=>error=message);viewport.update(ui,muted,traceId);onready(viewport);}catch(e){error=String(e);viewport?.dispose();}return()=>{onready(null);viewport?.dispose();};});
 $effect(()=>{viewport?.update(ui,muted,traceId);});
</script>
