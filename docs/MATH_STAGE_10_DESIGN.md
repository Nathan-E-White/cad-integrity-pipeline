...
an output that fails the existing kernel gate.  It copies selected faces into
...
OCCT sewing accepts shapes/faces, not caller-prescribed edge-to-edge mate
pairs.  This module therefore does not claim pair-level mate enforcement: that
would require a separate topology-reconstruction adapter.  Its honest selection
contract is the selected wire set, and unselected open faces are never passed to
the OCCT operation.
