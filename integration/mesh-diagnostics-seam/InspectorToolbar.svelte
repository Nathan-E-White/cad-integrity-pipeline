...
<div class="toolbar" aria-label="Mesh display controls">
  ...
  <label>Face scalar
    <select bind:value={state.fieldId}><option value={null}>No heatmap</option>{#each fields as field (field.id)}<option value={field.id}>{field.label}</option>{/each}</select>
  </label>
  ...
  {#if state.clipEnabled}
    <label>Clip axis <select bind:value={state.clipAxis}><option value={0}>X</option><option value={1}>Y</option><option value={2}>Z</option></select></label>
...
<style>
  ...
  input[type="range"] { width:100%; accent-color:#305982; } input[type="checkbox"] { accent-color:#305982; }
