...
struct FlatBVHNode {
    ...
    uint32_t primitive_count = 0; // Set to 0 for internal hierarchy nodes
    uint32_t second_child_offset = 0; // Right child escape offset index
};

// Allocation-Optimized Flat Bounding Volume Hierarchy Container
class FlatBVH {
public:
    ...
    static FlatBVH build(const TriangleMesh& mesh) noexcept {
        ...
        // Preallocate structural memory limits safely (2 * N - 1)
        ...
        while (idx < nodes.size()) {
            ...
            if (node.bounds.intersects(query_bounds)) {
                if (node.primitive_count > 0) {
                    ...
                    idx++; // Advance straight into the left child lane
                }
            } else {
                if (node.primitive_count > 0) {
                    idx++;
                } else {
                    // Escape route: skip left branch subtree entirely, jump to right sibling
...
private:
    size_t build_recursive(const TriangleMesh& mesh, size_t start, size_t end) noexcept {
        ...
        // Choose axis split criteria based on largest centroid extents
        int axis = 0;
        ...
        // In-place partition swapping without auxiliary runtime buffers
        size_t mid = start;
        for (size_t i = start; i < end; ++i) {
            size_t tri_idx = primitive_indices[i];
            const auto& tri = mesh.triangles[tri_idx];
        ...
        // Fallback split alignment if centroids are clustered closely together
...
int main() {
    // Generate a simple triangle mesh configuration
    ...
    std::cout << "Step 2: Triangulating BRep structure to TriangleMesh...\n";
    ...
    std::cout << "Step 3: Building a Flat BVH to accelerate spatial operations...\n";
    FlatBVH bvh = FlatBVH::build(mesh);
    std::cout << "  BVH complete. Total linear nodes allocated: " << bvh.nodes.size() << "\n";

    // Setup an exploratory query bounding box window
    AABB query_window;
    query_window.grow({0.1f, 0.1f, -0.1f});
    query_window.grow({0.9f, 0.9f,  0.1f});

    std::vector<size_t> query_results;
    bvh.query_box(query_window, query_results);
    std::cout << "  Spatial partition window search located " << query_results.size() << " overlapping primitive(s).\n\n";

    std::cout << "Step 4: Downward closure mapping to create the final SimplicialComplex...\n";
