...
public:
    ...
    {
        // Establish the datum reference frame orthogonal basis vectors
        Point3D<T> w = primary_normal.normalize(); // Z-axis aligns with primary datum normal
        Point3D<T> u = (secondary_direction - w * secondary_direction.dot(w)).normalize(); // X-axis
        Point3D<T> v = w.cross(u).normalize(); // Y-axis

        // Construct rigid transformation matrix mapping local datum coordinates to global coordinates
        return Matrix4x4{
...
// ============================================================================
// GEOMETRIC DIMENSIONING & TOLERANCING (GD&T) AUDITING LAYER
// ============================================================================
...
class GDTInspector {
public:
    // Evaluates parallelism error relative to an oriented primary reference datum axis
    [[nodiscard]] static constexpr GDTReport<T> evaluate_parallelism(
        const Point3D<T>& feature_normal,
        const Point3D<T>& datum_normal,
        T tolerance_zone) noexcept
    {
        Point3n<T> n_f = feature_normal.normalize();
        Point3D<T> n_d = datum_normal.normalize();

        // Deviation from zero-angle tilt calculated via exterior cross length mapping
        T deviation = n_f.cross(n_d).length();
        return GDTReport<T>{"Parallelism (||)", deviation, tolerance_zone, deviation <= tolerance_zone};
    }

    // Evaluates perpendicularity error relative to an oriented reference datum axis
        ...
        T tolerance_zone) noexcept
    {
        ...
        // Deviation from exact orthogonal mapping tracked via inner vector metrics
        T deviation = std::abs(n_f.dot(n_d));
        return GDTReport<T>{"Perpendicularity (|_|)", deviation, tolerance_zone, deviation <= tolerance_zone};
    }
};
...
int main() {
    std::cout << "=== Metrology Quality Pipeline & Spatial Acceleration ===\n\n";
    ...
    std::cout << "Indexing mesh elements inside an AABB partition tree...\n";
    ...
    std::cout << std::format("  -> Partition spatial tracking nodes generated: {}\n\n", bvh_nodes.size());

    // 3. Extracting Reference Datum Features via Local Analytical Fitting
    std::cout << "Isolating primary reference face components...\n";
    std::vector<std::size_t> primary_patch = {1, 2, 3, 4};
    Plane3D<double> datum_A_plane = fit_plane_primitive(positions, primary_patch);

    std::cout << std::format("  -> Discovered Datum A Normal: ({:.4f}, {:.4f}, {:.4f})\n",
                             datum_A_plane.normal.x, datum_A_plane.normal.y, datum_A_plane.normal.z);

    // 4. Rigid Datum Reference Frame Matrix Assembly
    Point3D<double> secondary_dir{1.0, 0.0, 0.0}; // Nominal layout guide directional vector
    Point3D<double> origin_datum = positions[1];  // Lock origin translation at first corner point

    std::cout << "Assembling rigid 4x4 Datum Alignment orientation matrix...\n";
    Matrix4x4<double> alignment_mat = Matrix4x4<double>::build_datum_alignment(datum_A_plane.normal, secondary_dir, origin_datum);

    // Transform a feature position vector directly into aligned engineering space
    Point3D<double> aligned_apex = alignment_mat * positions[0];
    std::cout << std::format("  -> Aligned Top Feature Vector: {}\n\n", aligned_apex);

    // 5. Automated GD&T Metrology Extra-Credit Validation Loops
    std::cout << "=== Executing GD&T Metrology Inspection Matrix ===\n";

    // Evaluate parallelism of an arbitrary upper element face against the primary datum base surface normal
    Point3D<double> test_face_normal{0.005, 0.002, 0.999};
    double tolerance_zone = 0.01; // 10 micron structural tolerance envelope limit

    GDTReport<double> parallelism_report = GDTInspector<double>::evaluate_parallelism(
        test_face_normal, datum_A_plane.normal, tolerance_zone
    );

    std::cout << std::format("  - Control: {}\n    Measured Deviation: {:.6f}\n    Allowed Limit:      {:.6f}\n    Status:             {}\n",
                             parallelism_report.symbol, parallelism_report.measured_deviation,
                             parallelism_report.tolerance_zone, parallelism_report.passed ? "PASSED" : "FAILED");

    // Evaluate perpendicularity of a secondary lateral structural face
    Point3D<double> side_flange_normal{1.002, 0.012, 0.001};
    GDTReport<double> perp_report = GDTInspector<double>::evaluate_perpendicularity(
        side_flange_normal, datum_A_plane.normal, tolerance_zone
    );

    std::cout << std::format("\n  - Control: {}\n    Measured Deviation: {:.6f}\n    Allowed Limit:      {:.6f}\n    Status:             {}\n",
                             perp_report.symbol, perp_report.measured_deviation,
                             perp_report.tolerance_zone, perp_report.passed ? "PASSED" : "FAILED");
