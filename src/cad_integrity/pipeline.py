"""Policy-consistent polygonal repair transition, not a CAD healing engine."""
...
class RepairPolicy:
    """One audit policy for every polygonal repair observation and transform."""
    weld: WeldPolicy | None = None
...
class RepairPipeline:
    """Own one configured audit/transform/recheck transition and its evidence trace."""
    def __init__(self, policy: RepairPolicy = RepairPolicy()) -> None:
        self.policy = policy

    def _analyze(self, candidate: PolyhedralBRep) -> TopologyReport:
    ...
    @staticmethod
    def _require_admissible(report: TopologyReport) -> None:
        if not report.has_admissible_polygonal_cells:
            raise RepairRejected(report.homology_unavailable_reason
                                 or "Input is not an admissible polygonal cell complex")

    def weld(self, brep: PolyhedralBRep, weld_policy: WeldPolicy) -> WeldResult:
        before = self._analyze(brep)
        self._require_admissible(before)
        result = _weld_vertices(brep, weld_policy, before)
        return result.with_audits(before, self._analyze(result.candidate))

    def synchronize_orientations(self, brep: PolyhedralBRep) -> OrientationResult:
        before = self._analyze(brep)
        self._require_admissible(before)
        result = _synchronize_orientations(brep)
        return result.with_audits(before, self._analyze(result.candidate))

    def run(self, brep: PolyhedralBRep, *, on_event: Callable[[StageEvent], None] | None = None) -> RepairResult:
        ...
        try:
            ...
            if self.policy.weld is not None:
                ...
                welded = _weld_vertices(candidate, self.policy.weld, before)
                candidate, maximum_move = welded.candidate, welded.maximum_displacement
            ...
            if self.policy.synchronize_orientation:
                ...
                oriented = _synchronize_orientations(candidate)
            ...
            after = self._analyze(candidate)
            decision = "topology_checks_passed" if after.homology is not None and after.is_closed_oriented_2manifold else "needs_review"
            report = RepairReport("1.0", fingerprint(brep), before, after, decision, tuple(changes), maximum_move, brep.length_unit, ())
            event("complete", decision)
            return RepairResult(brep, candidate, report)
        except IntegrityError as exc:
            event("rejected", str(exc))
            report = RepairReport("1.0", fingerprint(brep), before, None, "rejected", tuple(changes), maximum_move, brep.length_unit, (str(exc),))
